...
# Количественный анализ текста, практика, часть I
...
### Способ 1. Использовать готовые корпуса с готовым интерфейсом (корпус Google Books с его Ngram Viewer, НКРЯ, корпуса в Sketch Engine и т.п.).
...
⛳ 💻 Задание: если вы никогда этого не делали — загрузите, пожалуйста, в Voyant любой крупный (не менее 50 кб) txt-файл с *русским* текстом. Можете создать сами, найти у себя, в крайнем случае — возьмите [этот](data/Dostoevsky_PrestuplenieINakazanie.txt).
...
Перед загрузкой я лемматизировал корпус фантастики (подробнее об этом на следующем занятии). Поэтому вордклауд, который вам по умолчанию выдает Voyant в левом окошке, в нашем случае основан на частотностях лемм ☝️
...
1. Визуализацию в виде облака
2. Частотности в виде таблицы
...
* Возможность загрузить словарь лемм (вот [такой(осторожно, 60 мб)](data/lemma_list_ru.txt)), чтобы тексты лемматизировались прямо внутри программы.
* Контрастивный анализ (выделение слов, которые характеризуют *ваш* корпус относительно какого-то референтного корпуса)

⛳ 💻 Задание: установите [AntConc](http://www.laurenceanthony.net/software/antconc/) (достаточно просто скачать его на компьютер)

### Способ 3. Do it yourself, dammit 🛠️👩‍💻

#### Как вы уже поняли, у всех готовых инструментов есть серьезные ограничения. 

Вы не можете запихнуть в AntConc или Voyant 1ГБ текста, не можете сделать внутри них всю нужную вам пред- или постобработку (например, лемматизацию).

А самое главное: **ВЫ ОСТАЕТЕСЬ ПОЛЬЗОВАТЕЛЕМ ПРОДУКТА**. А круто быть разработчиком, сам-себе-инженером.
Пока мы пользуемся каким-то продуктом, мы зависимы от него. Веб-приложение могут закрыть, программу перестанут поддерживать. 
И если что-то не работает, нам остается только грустить: ведь все спрятано под капотом (в крайнем случае вы можете завести issue на гитхабе разработчиков... но вряд ли вы будете готовы пофиксить баг сами и кинуть pull-request).
...
### Поэтому в следующий раз мы будем делать Distant  Reading в Питоне, как взрослые.