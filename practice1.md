...
# Разберемся, а что мы вообще можем?

Способы делать Distant Reading на компьютере, от базовых — к продвинутым:

1. Использовать готовые корпуса с готовым интерфейсом (корпус Google Books с его Ngram Viewer, НКРЯ, Sketch Engine и т.п.).
...
### Плюсы: легко, дешево, низкий порог входа 👶🍼
### Минусы: мы ограничены И составом корпуса, И функционалом интерфейса; не имеем прямого доступа к текстам 🔒🛑

2. Собирать корпус самому, но исследовать его при помощи специализированных программ — корпусных менеджеров. Примеры популярных корпусных менеджеров:
...
* [Корпус фантастики, собранный в прошлом году нынешним вторым курсом](https://voyant-tools.org/?corpus=ad00c40ec4549bc30c185cf7f65c9318). 
